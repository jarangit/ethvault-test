...
export function Navbar() {
  ...
  const [previewBalance, setPreviewBalance] = useState<typeof balances>();
  const navigation = [
    { name: "Dashboard", href: "/" },
  ...
  useEffect(() => {
    if (isConnected) {
      setPreviewBalance(balances);
    }
  }, [isConnected, balances]);

  const BalanceItem = ({ title, value }: { title: string; value: number }) => {
    return (
      <div className="flex justify-between">
        <div>{title}</div>
        <strong className="text-lightblue-700">
          {previewBalance?.eth ? value.toFixed(2) : ""}
        </strong>
      </div>
    );
  };
    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-200 ${
          scrolled
            ? "bg-[#67c4ff]/90 backdrop-blur-md shadow-md"
            : "bg-[#67c4ff]"
        }`}
      >
                      ...
                      <div className="px-2 py-2">
                        ...
                        {previewBalance && (
                          <div className="flex gap-2 flex-col">
                            <div className="text-sm font-medium text-lightblue-700">
                              Balance
                            </div>
                            <div className="bg-lightblue-50 p-2 rounded-md text-sm font-mono">
                              <BalanceItem
                                title="ETH"
          ...
          <div className="mobile-menu-links">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className={`text-white hover:text-lightblue-100 ${
